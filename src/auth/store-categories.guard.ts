...
@Injectable()
export class RoleStoreCategoriesGuard implements CanActivate {
  constructor(
  ...
  async canActivate(_context: ExecutionContext) {
    ...
    try {
      ...
      if (user_type !== 'merchant' && user_type !== 'admin')
        throw new UnauthorizedException(
          this.roleAuthError(
            user_type,
            'user_type',
      ...
      if (user_type == 'merchant' && level !== 'merchant') {
        throw new UnauthorizedException(
          this.roleAuthError(
            level,
            'level',
      ...
      _req.payload = Object.assign({}, res.data.payload);
